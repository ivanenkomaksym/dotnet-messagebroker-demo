...
async fn create_order(order: &Order) -> Result<(), lapin::Error>
{
    let order_created_event = OrderCreated { 
        ...
        creation_date_time: chrono::DateTime::parse_from_rfc3339(order.creation_date_time.try_to_rfc3339_string().unwrap().as_str()).unwrap().with_timezone(&Utc)
    };

    Ok(publisher::publish_event(ORDER_CREATED_EXCHANGE, QUEUE_NAME, order_created_event).await?)
}

async fn update_order(order: &Order) -> Result<(), lapin::Error>
{
...
async fn cancel_order(order_id: bson::Uuid) -> Result<(), lapin::Error>
{
...
async fn order_collected(order_id: bson::Uuid) -> Result<(), lapin::Error>
{
...
async fn return_order(order_id: bson::Uuid) -> Result<(), lapin::Error>
{
